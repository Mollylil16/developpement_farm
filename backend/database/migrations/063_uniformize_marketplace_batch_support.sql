...
-- NOTE: Si la colonne 'photos' existe déjà (migration ultérieure), la vue sera mise à jour dans la migration 064
DO $$
DECLARE
  required_columns TEXT[] := ARRAY['listing_type', 'subject_id', 'batch_id', 'pig_count', 'weight', 'producer_id', 'status'];
  missing_columns TEXT[];
  col TEXT;
  photos_column_exists BOOLEAN;
BEGIN
  -- Vérifier si la colonne photos existe (migration ultérieure)
  SELECT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'marketplace_listings' AND column_name = 'photos'
  ) INTO photos_column_exists;

  -- Si photos existe, ne pas créer la vue ici (elle sera créée dans la migration 064)
  IF photos_column_exists THEN
    RAISE NOTICE 'Colonne photos détectée. La vue sera mise à jour dans la migration 064.';
  ELSE
