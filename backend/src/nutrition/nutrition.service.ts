...
export class NutritionService {
   ...
   * avec les permissions appropriées (nutrition ou gestion_complete)
   */
  private async checkProjetOwnership(projetId: string, userId: string): Promise<void> {
    ...
    // ✅ Sinon, vérifier s'il est collaborateur actif avec permission 'nutrition'
    const collabResult = await this.databaseService.query(
      `SELECT id, permission_nutrition, permission_gestion_complete FROM collaborations 
       WHERE projet_id = $1 
    ...
    if (collabResult.rows.length > 0) {
      ...
      // Vérifier si la permission nutrition ou gestion_complete est accordée
      if (collab.permission_nutrition === true || collab.permission_gestion_complete === true) {
        return;
      }
    }
