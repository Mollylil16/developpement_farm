...
export class SanteService {
   ...
   * avec les permissions appropriées (sante ou gestion_complete)
   */
  private async checkProjetOwnership(projetId: string, userId: string): Promise<void> {
    ...
    // ✅ Sinon, vérifier s'il est collaborateur actif avec permission 'sante'
    ...
    if (collabResult.rows.length > 0) {
      ...
      // Vérifier si la permission sante ou gestion_complete est accordée
