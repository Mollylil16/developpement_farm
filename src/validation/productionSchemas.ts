...
export const animalSchema = yup.object().shape({
  code: yup
    ...
    .matches(/^[A-Z0-9-]+$/, 'Le code ne peut contenir que des lettres majuscules, chiffres et tirets'),
...
export const peseeSchema = yup.object().shape({
  animal_id: yup
    .string()
    .required('L\'animal est obligatoire'),
...
export const mortaliteSchema = yup.object().shape({
  ...
  cause: yup
    .string()
    .nullable()
    .max(200, 'La cause ne peut pas dépasser 200 caractères'),
