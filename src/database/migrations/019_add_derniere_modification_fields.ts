...
export async function addDerniereModificationFields(db: SQLiteDatabase): Promise<void> {
  ...
  try {
    ...
    const hasDerniereModification = revenusColumns.some(
      (col) => col.name === 'derniere_modification'
    );
  ...
  try {
    ...
    const hasDerniereModification = depensesColumns.some(
      (col) => col.name === 'derniere_modification'
    );
  ...
  try {
    ...
    if (marketplaceTableExists) {
      ...
      const hasDerniereModification = marketplaceColumns.some(
        (col) => col.name === 'derniere_modification'
      );
