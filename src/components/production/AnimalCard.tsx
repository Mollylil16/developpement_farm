  ...
  return (
    <Card elevation="small" padding="medium" style={styles.card}>
      <View style={styles.header}>
        {animal.photo_uri ? (
          <Image key={`photo-${animal.id}-${animal.photo_uri}`} source={{ uri: animal.photo_uri }} style={styles.photo} resizeMode="cover" />
        ) : (
          <View style={[styles.photo, styles.photoPlaceholder, { backgroundColor: colors.primaryLight + '15', borderColor: colors.primary + '30' }]}>
        ...
        <View style={styles.info}>
          ...
          <View style={styles.badgesRow}>
            <View style={[styles.statutBadge, { backgroundColor: statutColor + '15' }]}>
              <Text style={[styles.statutText, { color: statutColor }]}>{STATUT_ANIMAL_LABELS[animal.statut]}</Text>
            </View>
            {animal.reproducteur && (
            ...
            {(animal as ProductionAnimalWithMarketplace).marketplace_status === 'available' && (
              <View style={[styles.marketplaceBadge, { backgroundColor: '#FF8C42' + '20', borderColor: '#FF8C42' }]}>
            ...
            {(animal as ProductionAnimalWithMarketplace).marketplace_status === 'reserved' && (
              <View style={[styles.marketplaceBadge, { backgroundColor: '#F39C12' + '20', borderColor: '#F39C12' }]}>
      ...
      <View style={styles.actions}>
        ...
        {canUpdate && (
          <TouchableOpacity style={[styles.actionButton, { backgroundColor: colors.primary + '15' }]} onPress={() => onEdit?.(animal)}>
            <Text style={[styles.actionButtonText, { color: colors.primary }]}>Modifier</Text>
          </TouchableOpacity>
        )}
        {canDelete && (
          <TouchableOpacity style={[styles.actionButton, { backgroundColor: colors.error + '15' }]} onPress={() => onDelete?.(animal)}>
      ...
      <View style={styles.details}>
        ...
        {animal.date_naissance && (
          <View style={styles.detailRow}>
            ...
            <Text style={[styles.detailValue, { color: colors.text }]}>{format(parseISO(animal.date_naissance), 'dd MMM yyyy', { locale: fr })}</Text>
          </View>
        )}
        {animal.date_entree && (
          <View style={styles.detailRow}>
            <Text style={[styles.detailLabel, { color: colors.textSecondary }]}>Date d'arrivée:</Text>
            <Text style={[styles.detailValue, { color: colors.text }]}>{format(parseISO(animal.date_entree), 'dd MMM yyyy', { locale: fr })}</Text>
          </View>
        )}
        {animal.race && (
          <View style={styles.detailRow}>
        ...
        <View style={styles.detailRow}>
          ...
          <Text style={[styles.detailValue, { color: colors.text }]}>{getParentLabel(animal.pere_id)}</Text>
        </View>
        <View style={styles.detailRow}>
          <Text style={[styles.detailLabel, { color: colors.textSecondary }]}>Mère:</Text>
          <Text style={[styles.detailValue, { color: colors.text }]}>{getParentLabel(animal.mere_id)}</Text>
        </View>
        <View style={styles.detailRow}>
      ...
      {totalHistorique > 0 && (
        <>
          ...
          <View style={styles.historiqueContainer}>
            <TouchableOpacity style={styles.historiqueHeader} onPress={() => onToggleHistorique?.(animal.id)}>
              <View style={styles.historiqueHeaderLeft}>
                <Ionicons name="medkit" size={18} color={colors.primary} />
                <Text style={[styles.historiqueTitle, { color: colors.text }]}>Historique sanitaire</Text>
                <View style={[styles.historiqueBadge, { backgroundColor: colors.primary + '15' }]}>
                  <Text style={[styles.historiqueBadgeText, { color: colors.primary }]}>{totalHistorique}</Text>
                </View>
              </View>
            ...
            {isExpanded && (
              <View style={styles.historiqueContent}>
                      ...
                      return (
                        <View key={v.id} style={[styles.historiqueItem, { backgroundColor: colors.success + '08', borderLeftColor: colors.success }]}>
                          <View style={styles.historiqueItemHeader}>
                            ...
                            <View style={[styles.categorieBadge, { backgroundColor: colors.success + '20' }]}>
                              <Text style={[styles.categorieBadgeText, { color: colors.success }]}>{categorieLabel}</Text>
                            </View>
                          </View>
                ...
                {maladiesAnimal.length > 0 && (
                  <View style={styles.historiqueSection}>
                    <Text style={[styles.historiqueSectionTitle, { color: colors.warning }]}>🏥 Maladies ({maladiesAnimal.length})</Text>
                ...
                {traitementsAnimal.length > 0 && (
                  <View style={styles.historiqueSection}>
                    <Text style={[styles.historiqueSectionTitle, { color: colors.info }]}>💊 Traitements ({traitementsAnimal.length})</Text>
            ...
            {(['actif', 'mort', 'vendu', 'offert'] as StatutAnimal[]).map((statut) => (
              <TouchableOpacity
                ...
                style={[
                  styles.statutButton,
                  {
                    backgroundColor: animal.statut === statut ? getStatutColor(statut, colors) : colors.background,
              ...
              >
                <Text
                  style={[
                    ...
                    {
                      color: animal.statut === statut ? colors.textOnPrimary : getStatutColor(statut, colors),
                    },
                  ]}
                >
