...
export function useVaccinationLogic(): VaccinationLogicReturn {
  ...
  const statParType = useMemo((): StatistiquesProphylaxieParType[] => {
    ...
    return types.map((type) => {
      ...
      const effectues = vaccinationsType.filter((v) => v.statut === 'effectue').length;
      ...
      const totalAnimauxConcernes = animauxConcernes.size;
      const tauxCouverture =
        statsGlobales.totalAnimaux > 0
          ? Math.round((totalAnimauxConcernes / statsGlobales.totalAnimaux) * 100)
          : 0;
