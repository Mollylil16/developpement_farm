...
describe('UserDataService', () => {
  ...
  describe('clearUserData', () => {
    ...
    it('devrait supprimer toutes les données d\'un utilisateur avec projets', async () => {
      ...
      expect(deleteCalls.filter(call => call[0] === 'DELETE FROM rations WHERE projet_id = ?').length).toBe(2);
      ...
      expect(deleteCalls.filter(call => call[0].includes('calendrier_vaccination')).length).toBe(2);

      // Vérifier la suppression des projets
      expect(deleteCalls.some(call => 
        call[0] === 'DELETE FROM projets WHERE proprietaire_id = ?' && 
        call[1][0] === userId
      )).toBe(true);

      // Vérifier la suppression de l'utilisateur
      expect(deleteCalls.some(call => 
        call[0] === 'DELETE FROM users WHERE id = ?' && 
        call[1][0] === userId
      )).toBe(true);
    });
    ...
    it('devrait gérer un utilisateur sans projets', async () => {
      ...
      expect(deleteCalls.some(call => 
        call[0] === 'DELETE FROM collaborations WHERE user_id = ?'
      )).toBe(true);
      expect(deleteCalls.some(call => 
        call[0] === 'DELETE FROM users WHERE id = ?'
      )).toBe(true);
    ...
    it('devrait lancer une erreur si la suppression échoue', async () => {
      ...
      await expect(UserDataService.clearUserData(userId)).rejects.toThrow('Erreur de base de données');
    });
    ...
    it('devrait respecter l\'ordre de suppression des dépendances', async () => {
      ...
      const stocksMouvementsIndex = deleteCalls.findIndex(call => call.includes('stocks_mouvements'));
      ...
      const ingredientsRationIndex = deleteCalls.findIndex(call => call.includes('ingredients_ration'));
      const rationsIndex = deleteCalls.findIndex(call => call.includes('rations') && !call.includes('ingredients_ration'));
      expect(ingredientsRationIndex).toBeLessThan(rationsIndex);

      // Vérifier que les projets sont supprimés avant l'utilisateur
      const projetsIndex = deleteCalls.findIndex(call => call.includes('projets WHERE proprietaire_id'));
